...
Do not use a free model whose terms allow private debate data to be retained or used for training/improvement. Record the selected model IDs in `docs/DECISIONS.md` once confirmed. Do not embed API keys or model IDs in prompts or documentation; `AI_LAWYER_MODEL` and `AI_JUDGE_MODEL` are server-side configuration only.
